...
def sample_subcategory(subcategory_data, subcategory_samples, random_state=42):
    if len(subcategory_data) < subcategory_samples:
        return pd.DataFrame(columns=subcategory_data.columns)  # Return an empty DataFrame with the same columns
    return subcategory_data.sample(n=subcategory_samples, random_state=random_state)


def proportionate_stratified_sampling(df, prioritized_subcategories, total_samples, batch_size=1000):
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for category, subcategories in prioritized_subcategories.items():
            for subcategory in subcategories:
                ...
                if subcategory_samples > 0:
                    futures.append(executor.submit(sample_subcategory, subcategory_data, subcategory_samples))
        ...
        sampled_dfs = [future.result() for future in futures]
...
def perform_stratified_sampling(input_file, output_file, benchmark_percentages, total_samples=3000):
    try:
        ...
        # Filter infrequent subcategories
        for category, subcategories in benchmark_percentages.items():
            df = df[df[category].isin(subcategories)]
        ...
        # Perform proportionate stratified sampling in batches
...
# Perform proportionate stratified sampling and generate the comparison chart
perform_stratified_sampling('output_excel/test_file_30K__.xlsx', 'output_excel/testrun_3.xlsx', benchmark_percentages)
